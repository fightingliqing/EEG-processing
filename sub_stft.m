...
% /Input/
% x: the original data samples (Time points x trails)   信号矩阵，可以多个trails一起做
% xtimes: the time axis of the original data   x对应的时间信息
% t: evaluated time points in STFT   时间，以秒为单位，把毫秒转为秒
% f: evaluated frequency bins in STFT   时频区间，核兹
% Fs: sampling rate  采样率
% winsize: window size (NOTE: the unit is sec)  窗宽
% wintype: window type (default: hanning window)  窗的类型，推荐hang ming窗
...
% /Output/
% S: complex time-frequency value  复数
% U: a constant to normalize the power  常数
% P: squared magnitude without phase  核兹power
% F: phase information  相位
